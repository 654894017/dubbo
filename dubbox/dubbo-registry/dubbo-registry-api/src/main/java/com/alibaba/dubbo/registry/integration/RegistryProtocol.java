...
import com.alibaba.dubbo.common.LocalCaches;
...
public class RegistryProtocol implements Protocol {
    ...
    public <T> Exporter<T> export(final Invoker<T> originInvoker) throws RpcException {
        ...
        //增加parrot协议判断和修改(MaJ)
		final URL registedProviderUrl = getRegistedParrotUrlPath(
				getRegistedProviderUrl(originInvoker), Constants.PROVIDER);
    ...
    /**
     * 对于parrot协议对URL做特殊装换 (MaJ)
     * this method is TODO
     * @param url
     * @param caller
     * @return 如果为parrot协议，对url做转换，如果不是parrot协议，则直接返回
     * @createTime 2016年8月22日 下午6:46:06
     * @author root
     */
	private URL getRegistedParrotUrlPath(URL url, String caller) {
		String protocol = "";
		if (Constants.CONSUMER.equals(caller))
			protocol = url.getParameter(Constants.PROTOCOL_KEY);
		if (Constants.PROVIDER.equals(caller))
			protocol = url.getProtocol();
		if(protocol==null)
		    return url;
		if (!protocol.equals(Constants.PARROT_PROTOCOL))// 新增parrot协议判断(MaJ)
			return url;
		String name = url.getServiceInterface();
		try {
			Class<?> c = Class.forName(name);
			Class<?> thriftClass;
			Annotation[] classAnnotation = c.getAnnotations();
	         for(Annotation cAnnotation : classAnnotation){
	              Class<?> annotationType =  cAnnotation.annotationType();
	              System.out.println("User类上的注释有: " +annotationType.getName());
	              System.out.println("aaa:"+com.facebook.swift.service.ThriftService.class.getName());
	              if(com.facebook.swift.service.ThriftService.class.getName().equals(annotationType.getName())){
	                  thriftClass=Class.forName(annotationType.getName());
	              }
	         }
	        
			for (Method method : c.getAnnotation(ThriftService.class)
					.annotationType().getDeclaredMethods()) {
				Object invoke = method.invoke(c
						.getAnnotation(ThriftService.class));
				name = invoke.toString();
				break;
			}
		} catch (Throwable e) {
			throw new RpcException("Failed to toParrotServicePath :" + url
					+ ", cause: " + e.getMessage(), e);
		}
		url = url.setPath(name);
		url = url.setServiceInterface(name);
		return url;
	}
    ...
    private <T> Invoker<T> doRefer(Cluster cluster, Registry registry, Class<T> type, URL url) {
        ...
        //增加parrot协议判断和修改(MaJ)
		subscribeUrl = getRegistedParrotUrlPath(subscribeUrl,
				Constants.CONSUMER);
